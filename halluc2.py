...
def split_sents(t):
    ...
    for p in re.split(r"(?<=[.!?])\s+|\n+", t):
        ...
        if len(p) >= 12 and re.search(r"[가-힣]", p): out.append(p)
    return out
...
def find_parens(ans, src):
    ...
    for m in re.finditer(r"([가-힣A-Za-z]{2,12})\s*\(([^)]{4,60})\)", ans):
        ...
        toks = [w for w in re.findall(r"[가-힣]{2,}", expl)]
...
검사할 문장의 내용을 뒷받침하는 근거를 원문에서 찾아 그대로 옮겨 적으세요.
원문에 없는 내용이면 '근거없음'이라고만 적으세요.
추측하거나 새로 만들어 쓰지 마세요.
...
for gm in prev:
    ...
    for dname, src in SOURCES.items():
        ...
        for s in sents:
            ...
            # 인용 검증 — 판사가 인용한 문장이 실제 원문에 있는가
            c = norm(cite)[:60]
            if len(c) < 10 or c[:20] not in src_n:
                bad.append((s, f"인용 불일치: {cite[:40]}"))
...
json.dump(report, open("/home/hufs/이윤우/halluc2_report.json","w"), ensure_ascii=False, indent=2)
print("\n저장: ~/이윤우/halluc2_report.json")
